    ...
    async (dispatch, getState) => {
        try {
            ...
            const { data } = await axios.get(
                `/api/user/all/users?currentPage=${currentPage}`,
                config
            )
...
export const editSingleUser = (fields) => async (dispatch, getState) => {
    try {
        ...
        dispatch({
            ...
            payload: { ...data.updatedUser, token: user.token },
        })

        localStorage.setItem(
            "user",
            JSON.stringify({ ...data.updatedUser, token: user.token })
        )
    } catch (error) {
        const err =
            error.response && error.response.data.message
