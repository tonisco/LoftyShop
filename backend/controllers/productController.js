...
const getShopProducts = asyncHandler(async (req, res) => {
    const { vendor, category, gender, minPrice, maxPrice, currentPage } =
        req.query
...
const getCartProductById = asyncHandler(async (req, res) => {
    ...
    const product = await Product.findById(id).select(
        "name image price discount countInStock"
    )
...
const getRelatedProducts = asyncHandler(async (req, res) => {
    ...
    const relatedProduct = await Product.find({ gender, _id: { $ne: id } })
        ...
        .limit(4)
    await Vendor.populate(relatedProduct, {
        path: "vendor",
        select: "companyName image",
    })
